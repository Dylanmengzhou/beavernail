...
export default function AppointmentReminder() {
	...
	useEffect(() => {
		const fetchNextReservation = async () => {
			try {
				const response = await fetch('/api/reservations/approachingReservationTime');
				if (!response.ok) {
				...
				if (data.nextReservation) {
					...
					setNextReservation({
						date: `${reservationDate.getFullYear()}.${String(reservationDate.getMonth() + 1).padStart(2, '0')}.${String(reservationDate.getDate()).padStart(2, '0')}`,
	...
	return (
		<div className="w-80 h-fit bg-white rounded-t-2xl rounded-bl-2xl p-2 px-4 border-2 ">
			<div className={`${zcool.className} text-[#fa5e75] flex justify-between`}>
			...
			{nextReservation ? (
				<>
					<div className={`${coiny.className} text-[#424242] text-2xl flex items-center gap-2`}>
						<span>{nextReservation.date}</span>
						<span className={`${zcool.className} text-xl`}>{nextReservation.weekDay}</span>
					</div>
					<div className={`${coiny.className} text-[#424242] text-xl`}>
