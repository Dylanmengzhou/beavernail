...
export default function ReservationDetailPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    const fetchReservationDetail = async () => {
      try {
        const response = await fetch(`/api/reservations/history/${reservationId}`);
    ...
    return (
      <div className="w-11/12 pt-4 md:py-6 px-4 md:px-6 h-full bg-amber-200 rounded-t-3xl">
        <div className="flex items-center mb-4">
          ...
          <h1 className="text-xl md:text-2xl font-bold ml-2">预约详情</h1>
        </div>
      ...
      <Card className="bg-white rounded-lg shadow-sm border-gray-100">
        <CardHeader className="pb-2">
          <div className="flex justify-between items-center">
            ...
            <Badge
              variant="outline"
              className={`${statusMap[reservation.status].className} px-2 py-1`}
            >
            ...
            <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
              <DialogTrigger asChild>
                <Button
                  ...
                  title={isWithin24Hours ? "预约无法取消：距离预约时间不足24小时（韩国时间）" : ""}
                >
                  取消预约
                </Button>
              </DialogTrigger>
